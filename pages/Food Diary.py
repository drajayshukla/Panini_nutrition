...
def main():
    ...
    **Instructions:**  
    Answer the questions below for three days, followed by additional notes about your eating experience.
    """)
    ...
    for day in days:
        ...
        for meal_time in ["Breakfast", "Mid-morning Snack", "Lunch", "Afternoon Snack", "Dinner"]:
            ...
            food_item = st.text_input(f"What did you eat for {meal_time}?", key=f"{day}_{meal_time}_food")
            portion = st.text_input(f"Quantity/Portion for {meal_time}?", key=f"{day}_{meal_time}_portion")
