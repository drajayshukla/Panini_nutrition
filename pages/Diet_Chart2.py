...
def select_items(meal_type, mandatory_subtypes, optional_subtypes=None, prob_random=None, veg_only=False):
    items = []
...
def generate_daily_chart(total_calories, calorie_distribution, veg_only):
    ...
    for meal_type, (mandatory_subtypes, optional_subtypes, prob_random) in meal_structure.items():
        items = select_items(meal_type, mandatory_subtypes, optional_subtypes, prob_random, veg_only)
        for item in items:
...
def generate_monthly_chart(total_calories, calorie_distribution, veg_only):
    monthly_chart = []
    for _ in range(30):  # 30 days in a month
...
def create_pdf(monthly_chart, file_path, total_calories, calorie_distribution, veg_only):
    ...
    # Add summary for daily calorie surplus or deficit
    pdf.set_font("Arial", size=12, style="B")
    pdf.cell(0, 10, "Daily Calorie Surplus or Deficit:", ln=True)
    pdf.set_font("Arial", size=10)
    for day, daily_data in enumerate(monthly_chart, start=1):
        ...
        if surplus_deficit > 0:
            pdf.cell(0, 10, f"Day {day}: Surplus of {surplus_deficit:.2f} kcal", ln=True)
        elif surplus_deficit < 0:
            pdf.cell(0, 10, f"Day {day}: Deficit of {abs(surplus_deficit):.2f} kcal", ln=True)
        else:
            pdf.cell(0, 10, f"Day {day}: Balanced at {total_calories} kcal", ln=True)
    ...
    for day, daily_data in enumerate(monthly_chart, start=1):
        ...
        pdf.cell(0, 10, f"Day {day} - Diet Chart", ln=True, fill=True)
        pdf.set_font("Arial", size=10)
        ...
        for section, subcodes in sections.items():
            ...
            pdf.cell(60, 10, "Food Item", border=1, fill=True, align='C')
            pdf.cell(40, 10, "Quantity", border=1, fill=True, align='C')
            pdf.cell(40, 10, "Calories (kcal)", border=1, fill=True, align='C')
            pdf.cell(40, 10, "Protein (g)", border=1, fill=True, align='C')
            pdf.ln()

            # Add items for the section
            for item in daily_data["daily_chart"]:
                if item["subcode"] in subcodes:
                    pdf.cell(60, 10, item["Name"], border=1, align='L')
...
total_calories = st.number_input("Enter your total daily calorie requirement (kcal):", min_value=500, max_value=4000, step=50, value=1000)
...
if sum(calorie_distribution.values()) > total_calories:
    ...
    if st.button("Generate Diet Chart"):
        ...
        with open(output_pdf_path, "rb") as pdf_file:
            st.download_button("Download Diet Chart PDF", pdf_file, "monthly_diet_chart2.pdf")
